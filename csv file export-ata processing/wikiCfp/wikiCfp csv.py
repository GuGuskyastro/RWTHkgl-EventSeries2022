...
cursor = conn.cursor()
cursor.execute("select title, acronym, city, country, startDate, endDate, year, seriesId, url,wikiCfpid from event_wikicfp")
with open('wikiCfpEvent.csv', 'w', encoding='utf-8', newline="") as f:
    writer = csv.writer(f)
    writer.writerow(['title', 'acronym','city', 'country', 'startDate', 'endDate', 'year', 'seriesId','url','wikiCfpid'])
