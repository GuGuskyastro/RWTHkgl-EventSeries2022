...
import sqlite3
import pandas as pd
from ParsingAndNormalization import dateParse
...
cursor = conn.cursor()
cursor.execute("select title, acronym, city, country, startDate, endDate, year, eventId, url from event_dblp")
with open('dblpEvent.csv', 'w', encoding='utf-8', newline="") as f:
    writer = csv.writer(f)
    writer.writerow(['title', 'acronym','city', 'country', 'startDate', 'endDate', 'year', 'eventId','url'])
