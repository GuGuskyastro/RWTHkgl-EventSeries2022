...
cursor = conn.cursor()
cursor.execute("select title, acronym, ordinal, city, country, startDate, endDate, year, wikiCfpid, dblpid, url from event_wikidata")
with open('wikidataEvent.csv', 'w', encoding='utf-8', newline="") as f:
    writer = csv.writer(f)
    writer.writerow(['title', 'acronym', 'ordinal', 'city', 'country', 'startDate', 'endDate', 'year', 'wikiCfpid', 'dblpid','url'])
